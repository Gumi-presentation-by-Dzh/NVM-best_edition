...
TEST(){
    ...
    for ((i = 0; i <= $N; i += 1)); do
        ...
        echo -e >> $(pwd)/$dir_name/time.log
        echo "NVM read latency : $NVM_READ_CLAT ns" >> $(pwd)/$dir_name/memtest.log
        echo "NVM read latency : $NVM_READ_CLAT ns" >> $(pwd)/$dir_name/time.log
        echo "NVM write latency : $NVM_WRITE_CLAT ns" >> $(pwd)/$dir_name/memtest.log
        echo "NVM write latency : $NVM_WRITE_CLAT ns" >> $(pwd)/$dir_name/time.log
        CHANGE_READ
        time (sh runenv.sh memtester -p 0x880000000 500K 1 >> $(pwd)/$dir_name/memtest.log)>>$(pwd)/$dir_name/time.log 2>&1
    done
}
